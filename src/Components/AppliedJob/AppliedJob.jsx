...
const AppliedJob = ({job}) => {   
   let {id, logo, job_title, company_name, remote_or_onsite, job_type, location, salary} = job

   return (
      <div className="border border-[#E8E8E8] rounded-md p-4 flex gap-5">
         <div className="w-2/12">
            <div className="bg-gray-200 rounded h-full flex items-center p-8">
               <img src={logo} className="w-full" />
            </div>
         </div>
         <div className="flex justify-between items-center w-10/12">
            <div>
               <h2 className='text-xl font-bold text-[#474747]'>{job_title}</h2>
               <p className='text-[#757575] text-lg mt-1 mb-2'>{company_name}</p>
               <div className="flex gap-4">
                  <button className='btn btn-outline btn-sm border-[#7E90FE] rounded-md text-[#907BFE]'>{remote_or_onsite}</button>
                  <button className='btn btn-outline btn-sm border-[#7E90FE] rounded-md text-[#907BFE]'>{job_type}</button>
               </div>
               <div className="flex gap-4 my-3">
                  <div className='flex items-center gap-2'>
                     ...
                     <p className='text-[#757575] font-medium'>{location}</p>
                  </div>
                  <div className='flex items-center gap-2'>
                     <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none">
                     ...
                     <p className='text-[#757575] font-medium'>Salary : {salary}</p>
                  </div>            
               </div>
            </div>
            <Link to={`/job/${id}`}>
               <button className='bg-gradient-to-r from-[#7E90FE] to-[#9873FF] text-white py-2 px-4 font-medium rounded-md'>View Details</button>
            </Link>
         </div>
      </div>
   );
